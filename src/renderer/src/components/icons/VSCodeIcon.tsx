...
/**
 * Official VS Code icon from the Visual Studio Code brand assets.
 * Uses unique IDs to avoid SVG def conflicts when rendered multiple times.
 * SVG source: https://code.visualstudio.com/brand
 */
export function VSCodeIcon({ size = 13, className }: { size?: number; className?: string }) {
  // Using a unique ID prefix to avoid clashes if multiple instances render
  const id = React.useId()
  const maskId = `vscode-mask-${id}`
  const filter0Id = `vscode-filter0-${id}`
  const filter1Id = `vscode-filter1-${id}`
  const gradientId = `vscode-gradient-${id}`
